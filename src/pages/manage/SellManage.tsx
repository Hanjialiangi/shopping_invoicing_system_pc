...
const pageSize = 5;
export function SellManage():JSX.Element{
    useDocumentTitle('商场进存销管理系统——销售信息')
  ...
  const [data,setData] = useState<any>(); //表格数据
    ...
    const columns = [
        ...
        { title: '销售数量',dataIndex:'number',key:'number'},
        ...
        <Button type="link"><Link to={`/sell/edit/${record.id}`}>编辑</Link></Button>
  ...
  >
     ...
     <Button type="primary" className="add"><Link to="/sell/add">新增</Link></Button>
     <Table
